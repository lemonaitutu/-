...
result_static = platform_odo_1day_core('Beijing 4th-ring rectangle route', [0; 0; 0], [0; 0; 30]*arcdeg, 11);
% 保存 result_static 结构体，里面包含误差曲线、滤波状态、传感器误差参数和均方误差。
save('platform_odo_1day_static_single_results.mat', 'result_static');
...
% Cbn0 是导航系速度投影到载体系/里程计坐标系的方向余弦矩阵，用作里程计观测矩阵。
    Cbn0 = q2mat(qnb0)';
% Odometer installation error angles relative to the vehicle/body frame.
% Set to zeros(3,1) to remove the installation error.
    odoInstallErr = [0.2; -0.1; 0.3]*arcdeg;
    Cbo = rv2m(odoInstallErr);
    CbnOdo = Cbo*Cbn0;
% IMU installation error angles relative to the platform/body frame.
% Set to zeros(3,1) to remove the IMU installation error.
    imuInstallErr = [0.05; -0.03; 0.08]*arcdeg;
    Cimu = rv2m(imuInstallErr);
% 初始参考位置：[纬度; 经度; 高度]，纬经度用弧度，高度用米。这里约为西安附近 34°N、108°E、高度 100 m。
    [posRef, vnRef, attRef] = fourth_ring_route_state(route, 0);
...
% Apply IMU installation error before adding sensor bias and random noise.
        wm = (Cimu*wm')';
        vm = (Cimu*vm')';
        [wm1, vm1] = imuadderr(wm, vm, eb, web, db, wdb, ts);
% Generate/acquire odometer output together with the sensor outputs.
% The odometer measurement is still produced at 1 Hz.
        if mod(t,1) < nts
            odo = CbnOdo*vnRefNext + rk.*randn(3,1);  % odometer output
        end
...
% 把当前惯导估计速度从导航系投影到车体系，得到里程计应观测到的速度 vb。
            vb = CbnOdo*vn;
% odo has been generated/acquired in module 2; this block only uses it.
...
function plot_fourth_ring_route(result)
    ...
    figure('Name', 'Actual vehicle trajectory');
    ...
    legend('Actual vehicle trajectory', 'INS/odometer navigation trajectory', 'Start / Wukesong anchor', ...
           'Initial direction', 'Location', 'best');
    ...
    sgtitle(sprintf('Actual simulated vehicle trajectory with driving disturbance, R=%.0f m, v=%.1f m/s', ...
            route.turnRadius, route.speed));
    exportgraphics(gcf, 'beijing_fourth_ring_actual_vehicle_trajectory.png', 'Resolution', 200);
end
