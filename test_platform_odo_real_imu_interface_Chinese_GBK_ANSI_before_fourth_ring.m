...
initialAlignAtt = [-0.00733; -0.000133; 6.2593];  % [roll; pitch; yaw], rad
result_static = platform_odo_real_imu_core('Static base real IMU', [0; 0; 0], initialAlignAtt, 11);
% 保存 result_static 结构体，里面包含误差曲线、滤波状态、传感器误差参数和均方误差。
save('platform_odo_real_imu_static_results.mat', 'result_static');
...
% 里程计量测仍按 1 Hz 频率产生。
        if mod(t,1) < nts
            odo = CbnOdo*vnRef + rk.*randn(3,1);  % 里程计输出
        end
