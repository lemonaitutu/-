...
% 已补充中文注释；文件保存为 GBK/ANSI 编码，便于 MATLAB R2019 中文环境打开。
% ============================================================
...
%   simulation_fourth_ring : simulated IMU + simulated odometer on a rectangular lap
%   simulation_static      : simulated IMU + simulated odometer on a static base
%   simulation_straight    : simulated IMU + simulated odometer on a straight line
%   real_mat               : real MAT IMU input, with the same navigation/filter modules
runCfg = make_default_run_config('real_mat');
initialAlignAtt = [0; 0; 0];  % [roll; pitch; yaw], rad; simulation route sets the heading internally.
result_static = platform_odo_real_imu_core(runCfg.sceneName, [0; 0; 0], initialAlignAtt, 11, runCfg);
% 保存 result_static 结构体，里面包含误差曲线、滤波状态、传感器误差参数和均方误差。
save(runCfg.outputFile, 'result_static');
...
% ============================================================
function result = platform_odo_real_imu_core(sceneName, vnRef, attb0, seed, runCfg)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 模块一：初始化模块
...
% 这里 nn=2、ts=0.1 s，所以导航状态每 0.2 s 更新一次。
    ...
    nn = 1;
    nts = nn*ts;
    if nargin < 5 || isempty(runCfg)
        runCfg = make_default_run_config('simulation_fourth_ring');
    end
    runCfg = normalize_run_config(runCfg, ts, nts);
    imuCfg = runCfg.imuCfg;
% 四环矩形路线仿真总时长按实际走完一圈设置。
% 一圈长度约 61.9 km，速度 15 m/s 时总时长约 1.15 h。
    scenario = init_motion_scenario(runCfg.scenario, arcdeg);
    simTime = scenario.simTime;
    route = scenario;
% 参考平台姿态四元数 qnpRef：单位四元数表示平台坐标系与导航系完全重合。
    [posRef, vnRef, attRef, trueEnu0] = motion_scenario_state(scenario, 0);
    qnpRef = [1; 0; 0; 0];
...
% 初始参考位置：[纬度; 经度; 高度]，纬经度用弧度，高度用米。这里约为五棵松附近 34°N、108°E、高度 100 m。
...
% 初始化卡尔曼滤波器结构体，包括 Xk、Pk、Qk、Rk、状态转移矩阵和观测矩阵。
    ...
    imuReader = [];
    if strcmpi(runCfg.imuMode, 'real')
        ...
        if strcmpi(imuCfg.source, 'mat')
            ...
            scenario.simHours = simTime/3600;
            route = scenario;
...
% 用参考位置和参考速度计算地球参数，作为理想 IMU 增量的来源。
        posRefPrev = posRef;
        vnRefPrev = vnRef;
        [posRefNext, vnRefNext, attRef, trueEnuNext] = motion_scenario_state(scenario, t);
        CbnOdo = Cbo*q2mat(a2qua(attRef))';
...
% 每经过约 1 小时在命令行打印一次进度。当前 simTime=3600，所以结束时打印 1 h。
    ...
    if strcmpi(runCfg.imuMode, 'real') && strcmpi(imuCfg.source, 'text') && ~isempty(imuReader) && imuReader.fid > 0
...
% ============================================================
function runCfg = make_default_run_config(mode)
    if nargin < 1 || isempty(mode)
        mode = 'simulation_fourth_ring';
    end

    runCfg.mode = mode;
    runCfg.imuMode = 'simulation';
    runCfg.odoMode = 'simulation';
    runCfg.sceneName = 'Beijing 4th-ring rectangle simulation';
    runCfg.outputFile = 'platform_odo_fourth_ring_sim_results.mat';

    runCfg.scenario.type = 'fourth_ring';
    runCfg.scenario.latDeg = 39.907456;
    runCfg.scenario.lonDeg = 116.273987;
    runCfg.scenario.h = 50;
    runCfg.scenario.speed = 15;
    runCfg.scenario.turnRadius = 300;
    runCfg.scenario.simTime = [];
    runCfg.scenario.att0 = [0; 0; 0];
    runCfg.scenario.vn = [15; 0; 0];

    if strcmpi(mode, 'simulation_static')
        runCfg.sceneName = 'Static base simulation';
        runCfg.outputFile = 'platform_odo_static_sim_results.mat';
        runCfg.scenario.type = 'static';
        runCfg.scenario.latDeg = 34;
        runCfg.scenario.lonDeg = 108;
        runCfg.scenario.h = 100;
        runCfg.scenario.simTime = 24*3600;
        runCfg.scenario.vn = [0; 0; 0];
    elseif strcmpi(mode, 'simulation_straight')
        runCfg.sceneName = 'Straight-line vehicle simulation';
        runCfg.outputFile = 'platform_odo_straight_sim_results.mat';
        runCfg.scenario.type = 'straight';
        runCfg.scenario.latDeg = 39.907456;
        runCfg.scenario.lonDeg = 116.273987;
        runCfg.scenario.h = 50;
        runCfg.scenario.speed = 15;
        runCfg.scenario.vn = [15; 0; 0];
        runCfg.scenario.simTime = 3600;
    elseif strcmpi(mode, 'real_mat')
        runCfg.sceneName = 'Real MAT IMU processing';
        runCfg.outputFile = 'platform_odo_real_imu_results.mat';
        runCfg.imuMode = 'real';
        runCfg.scenario.type = 'static';
        runCfg.scenario.latDeg = 34;
        runCfg.scenario.lonDeg = 108;
        ...
        runCfg.scenario.simTime = 3600;
        runCfg.scenario.vn = [0; 0; 0];
    elseif ~strcmpi(mode, 'simulation_fourth_ring')
        error('Unknown run mode: %s', mode);
    end

    runCfg.imuCfg.source = 'mat';
    runCfg.imuCfg.file = 'dataCS1034-C09#自瞄准好 HSP-2026-6-5-13-38-43_part1.mat';
    runCfg.imuCfg.matVariable = '';
    runCfg.imuCfg.rawTs = 0.002;
    runCfg.imuCfg.samplesPerUpdate = [];
    runCfg.imuCfg.accelCols = [1 2 3];
    runCfg.imuCfg.gyroCols  = [4 5 6];
    runCfg.imuCfg.accelUnit = 'mps2';
    runCfg.imuCfg.accelBiasMps2 = [0; 0; 0];
    runCfg.imuCfg.gyroBiasRadPerSec = [0; 0; 0];
    runCfg.imuCfg.autoEstimateGyroBias = true;
    runCfg.imuCfg.gyroBiasStaticSamples = 10000;
    runCfg.imuCfg.CdataToPlatform = [1 0 0; 0 0 -1; 0 1 0];
    runCfg.imuCfg.headerLines = 0;
    runCfg.imuCfg.accelScalePulsePerSecPerG = [1; 1; 1];
    runCfg.imuCfg.gyroScalePulsePerDeg = [1; 1; 1];
    runCfg.imuCfg.accelBiasG = [0; 0; 0];
    runCfg.imuCfg.gyroBiasDeg = [0; 0; 0];
end

function runCfg = normalize_run_config(runCfg, ts, nts)
    if ~isfield(runCfg, 'imuMode') || isempty(runCfg.imuMode)
        runCfg.imuMode = 'simulation';
    end
    if ~isfield(runCfg, 'odoMode') || isempty(runCfg.odoMode)
        runCfg.odoMode = 'simulation';
    end
    if ~isfield(runCfg, 'imuCfg') || isempty(runCfg.imuCfg)
        defaults = make_default_run_config('simulation_fourth_ring');
        runCfg.imuCfg = defaults.imuCfg;
    end
    runCfg.imuCfg.useRealData = strcmpi(runCfg.imuMode, 'real');
    if ~isfield(runCfg.imuCfg, 'samplesPerUpdate') || isempty(runCfg.imuCfg.samplesPerUpdate)
        runCfg.imuCfg.samplesPerUpdate = max(1, round(ts/runCfg.imuCfg.rawTs));
    end
    if ~isfield(runCfg, 'scenario') || isempty(runCfg.scenario)
        defaults = make_default_run_config('simulation_fourth_ring');
        runCfg.scenario = defaults.scenario;
    end
    if ~isfield(runCfg.scenario, 'simTime') || isempty(runCfg.scenario.simTime)
        runCfg.scenario.simTime = 3600;
    end
    if runCfg.scenario.simTime < nts
        error('Scenario simTime must be no shorter than one navigation update period.');
    end
end

function scenario = init_motion_scenario(cfg, arcdeg)
    scenario = cfg;
    scenario.pos0 = [cfg.latDeg*arcdeg; cfg.lonDeg*arcdeg; cfg.h];
    if ~isfield(scenario, 'att0') || isempty(scenario.att0)
        scenario.att0 = [0; 0; 0];
    end

    if strcmpi(cfg.type, 'fourth_ring')
        scenario = make_fourth_ring_route(cfg.latDeg*arcdeg, cfg.lonDeg*arcdeg, cfg.h, cfg.speed, cfg.turnRadius);
        scenario.type = 'fourth_ring';
        scenario.simTime = scenario.lapTime;
        scenario.simHours = scenario.simTime/3600;
        scenario.simLapCount = scenario.simTime*scenario.speed/scenario.lapDistance;
    elseif strcmpi(cfg.type, 'static')
        scenario.type = 'static';
        scenario.speed = 0;
        scenario.simTime = cfg.simTime;
        scenario.simHours = scenario.simTime/3600;
        scenario.simLapCount = 0;
    elseif strcmpi(cfg.type, 'straight')
        scenario.type = 'straight';
        scenario.simTime = cfg.simTime;
        scenario.simHours = scenario.simTime/3600;
        scenario.simLapCount = 0;
        if ~isfield(scenario, 'vn') || isempty(scenario.vn)
            scenario.vn = [cfg.speed; 0; 0];
        end
        scenario.speed = norm(scenario.vn(1:2));
    else
        error('Unknown scenario type: %s', cfg.type);
    end
end

function [posRef, vnRef, attRef, enu] = motion_scenario_state(scenario, t)
    if strcmpi(scenario.type, 'fourth_ring')
        [posRef, vnRef, attRef, enu] = fourth_ring_route_state(scenario, t);
    elseif strcmpi(scenario.type, 'static')
        enu = [0; 0; 0];
        posRef = scenario.pos0;
        vnRef = [0; 0; 0];
        attRef = scenario.att0;
    elseif strcmpi(scenario.type, 'straight')
        vnRef = scenario.vn(:);
        enu = vnRef*t;
        posRef = enu_to_pos(scenario.pos0, enu);
...
% 子图 6：滤波器估计的三轴加速度计零偏，单位 ug。
    ...
    sgtitle([result.sceneName, ' platform INS/odometer simulation']);
    if isfield(result.route, 'type') && strcmpi(result.route.type, 'fourth_ring')
...
% 真实 IMU 文本文件接口
% ============================================================
% 真实 IMU 的 MAT 文件读取接口 initRealImuMatReader
% 输入 cfg 为 imuCfg 配置结构体，输出 reader 为后续分块读取所需的数据缓存和索引。
% 本函数只负责打开并检查 MAT 文件，不在这里进行惯导积分。
% ============================================================
function reader = initRealImuMatReader(cfg)
% whos('-file',...) 只读取 MAT 文件变量信息，不把大数据全部加载到内存。
    fileInfo = whos('-file', cfg.file);
% 如果 MAT 文件为空，说明无法从该文件中获取 IMU 矩阵。
    if isempty(fileInfo)
        error('真实 IMU 的 MAT 文件中没有找到任何变量：%s', cfg.file);
    end

% cfg.matVariable 用于指定 MAT 文件中的变量名；为空时程序自动寻找合适矩阵。
    variableName = cfg.matVariable;
    if isempty(variableName)
% neededCols 是当前配置需要读取的最大列号，用于判断候选矩阵是否至少包含这些列。
        neededCols = max([cfg.accelCols(:); cfg.gyroCols(:)]);
        variableName = '';
% 遍历 MAT 文件中的变量，寻找二维数值矩阵作为 IMU 原始数据。
        for i = 1:numel(fileInfo)
            sizeInfo = fileInfo(i).size;
% 只考虑二维变量；既允许“行=采样点、列=通道”，也兼容转置存储的情况。
            if numel(sizeInfo) == 2 && (sizeInfo(2) >= neededCols || sizeInfo(1) >= neededCols)
% 只加载当前候选变量，避免一次性加载所有变量造成内存浪费。
                candidate = load(cfg.file, fileInfo(i).name);
% 只有数值型矩阵才能作为 IMU 数据使用。
...
% 如果自动搜索失败，则提示用户手动设置 imuCfg.matVariable 或检查列号配置。
        if isempty(variableName)
...
% 数据必须是二维数值矩阵，常见形式为 N×6 或 N×更多列。
    if ~isnumeric(data) || ndims(data) ~= 2
        error('MAT 变量 %s 必须是二维数值矩阵。', variableName);
    end
% 如果数据看起来是“通道×采样点”形式，则转置成“采样点×通道”。
    if size(data,2) < max([cfg.accelCols(:); cfg.gyroCols(:)]) && size(data,1) >= max([cfg.accelCols(:); cfg.gyroCols(:)])
        data = data.';
    end
% 转置后仍不满足列数要求，则说明 imuCfg.accelCols/gyroCols 设置与文件不匹配。
    if size(data,2) < max([cfg.accelCols(:); cfg.gyroCols(:)])
        error('MAT 变量 %s 不包含当前配置指定的 IMU 列。', variableName);
    end

% reader.data 保存完整原始数据，后续 readRealImuMatBlock 按 reader.index 顺序分块读取。
    reader.data = data;
% index 表示下一次读取从第几行开始。
    reader.index = 1;
% 记录实际采用的 MAT 变量名，便于调试和结果追踪。
    reader.variableName = variableName;
% 保存配置结构体，后续分块读取时仍要用列号、单位和坐标转换矩阵。
    reader.cfg = cfg;
    fprintf('使用真实 IMU 的 MAT 变量 %s，共 %d 个采样点。\n', variableName, size(data,1));
end

% ============================================================
% 真实 IMU 的 MAT 数据分块读取 readRealImuMatBlock
% 每次从 reader.data 中取 sampleCount 个原始采样点，合成为一个导航更新周期的 wm/vm。
% 输出 wm 为陀螺角增量，vm 为加速度计速度增量，二者均为 1×3 行向量。
% ============================================================
...
% 取出当前周期内的原始 IMU 数据，并转换为 double 便于数值计算。
    raw = double(reader.data(startIndex:endIndex,:));
% cfg 中包含通道列号、单位、零偏和坐标转换关系。
    cfg = reader.cfg;
% 读取加速度计三轴数据，并先扣除人工设置的加速度计标定零偏。
    accel = raw(:,cfg.accelCols) - cfg.accelBiasMps2(:).';
% 如果原始加速度单位是 g，则乘以 g0 转换成 m/s^2。
    if strcmpi(cfg.accelUnit, 'g')
        accel = accel*g0;
% 如果既不是 g 也不是 m/s^2，则说明配置有误，直接报错避免误积分。
    elseif ~strcmpi(cfg.accelUnit, 'mps2')
        error('imuCfg.accelUnit 必须设置为 ''g'' 或 ''mps2''。');
    end
% 读取陀螺三轴角速度，单位要求为 rad/s，并扣除陀螺零偏。
    gyroRadPerSec = raw(:,cfg.gyroCols) - cfg.gyroBiasRadPerSec(:).';

% 将数据文件坐标轴转换到平台/导航程序约定的 IMU 坐标轴。
    accelPlatform = (cfg.CdataToPlatform*accel.').';
    gyroPlatform = (cfg.CdataToPlatform*gyroRadPerSec.').';
% 加速度 m/s^2 乘以采样周期并累加，得到本导航周期速度增量 vm。
    vm = sum(accelPlatform*rawTs, 1);
% 角速度 rad/s 乘以采样周期并累加，得到本导航周期角增量 wm。
    wm = sum(gyroPlatform*rawTs, 1);
% 更新下一次读取起点。
    reader.index = endIndex + 1;
% 成功读取完整数据块，因此 EOF 标志为 false。
    isEof = false;
end

% ============================================================
% 真实 IMU 文本文件初始化 initRealImuReader
% 该接口用于读取普通文本/CSV 形式的脉冲数据；当前主程序默认使用 MAT 接口。
% ============================================================
function reader = initRealImuReader(cfg)
% 以只读方式打开真实 IMU 文本文件。
    fid = fopen(cfg.file, 'r');
% fid < 0 表示文件路径错误、文件不存在或无权限读取。
    if fid < 0
        error('无法打开真实 IMU 数据文件：%s', cfg.file);
    end
% 跳过文件头部说明行，使后续读取直接从数值数据开始。
    for i = 1:cfg.headerLines
% 如果文件还没跳完表头就结束，说明 headerLines 设置过大。
        if feof(fid)
            error('表头行数超过了真实 IMU 数据文件的实际长度。');
        end
        fgetl(fid);
    end
% 保存文件句柄和配置，供 readRealImuBlock 持续读取。
    reader.fid = fid;
    reader.cfg = cfg;
end

% ============================================================
% 真实 IMU 文本数据读取 readRealImuBlock
% 从文本文件连续读取 nn 行有效数值数据，并合成为一个导航更新周期的 wm/vm。
% 该函数适用于“加速度计/陀螺以脉冲计数形式输出”的数据格式。
% ============================================================
function [wm, vm, isEof] = readRealImuBlock(reader, nn, ts, g0, arcdeg)
% 取出文本接口配置，包括列号、比例因子和零偏。
    cfg = reader.cfg;
% 预分配每个子样的角增量和速度增量。
    wm = zeros(nn,3);
    vm = zeros(nn,3);
...
% row 记录已经成功读取的有效数值行数。
    row = 0;
% needCols 是判断一行文本是否包含足够 IMU 通道的最低列数要求。
    needCols = max([cfg.accelCols(:); cfg.gyroCols(:)]);

% 一直读取，直到获得 nn 行有效数据或读到文件末尾。
    while row < nn
% 文件结束时，只返回已经读到的有效样本，并通知主循环停止或截断。
        if feof(reader.fid)
            isEof = true;
            wm = wm(1:row,:);
            vm = vm(1:row,:);
...
% 如果该行数字列数不足，可能是空行、注释行或格式异常行，直接跳过。
...
% 将加速度计脉冲换算为 g 单位，并加入/扣除配置中的加速度零偏项。
% 注意这里的 cfg.accelScalePulsePerSecPerG 表示每秒每 g 对应的脉冲数。
        accG = accelPulse(:)' ./ (cfg.accelScalePulsePerSecPerG(:)'*ts) + cfg.accelBiasG(:)';
% 将陀螺脉冲换算为角度增量 deg，并加入/扣除配置中的陀螺零偏项。
        gyroDeg = gyroPulse(:)' ./ cfg.gyroScalePulsePerDeg(:)' + cfg.gyroBiasDeg(:)';

% 加速度 g 转为 m/s^2，再乘采样周期 ts，得到速度增量。
        vm(row,:) = accG*g0*ts;
% 角度 deg 转为 rad，得到角增量。
        wm(row,:) = gyroDeg*arcdeg;
    end
% 多个子样累加成一个导航更新周期的总角增量和总速度增量。
    wm = sum(wm,1);
    vm = sum(vm,1);
end

% ============================================================
% 文本行数字解析 parseNumericTextLine
% 将一行文本中的逗号、分号、制表符统一替换为空格，再用 sscanf 提取数字。
% ============================================================
function values = parseNumericTextLine(line)
% 兼容 MATLAB R2019 的 char/string 输入；非文本输入直接返回空数组。
    if ~ischar(line) && ~isstring(line)
        values = [];
        return;
    end
% 将 string 或 char 统一转换为 char，便于 strrep 和 sscanf 处理。
    text = char(line);
% 兼容 CSV、分号分隔和制表符分隔的文本格式。
    text = strrep(text, ',', ' ');
    text = strrep(text, ';', ' ');
    text = strrep(text, sprintf('\t'), ' ');
% sscanf('%f') 会提取当前行中按空白分隔的所有数字，最后转成行向量。
